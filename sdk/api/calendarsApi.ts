...
export class CalendarsApi {
     ...
     * @summary AddDateToCalendar: Add a date to a calendar
     ...
     * @summary BatchUpsertDatesForCalendar: Batch upsert dates to a calendar
     * @param scope Scope of the calendar
     * @param code Code of the calendar
     ...
     */
    public async batchUpsertDatesForCalendar (scope: string, code: string, successMode: string, requestBody: { [key: string]: CreateDateRequest; }, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: BatchUpsertDatesForCalendarResponse;  }> {
        const localVarPath = this.basePath + '/api/calendars/generic/{scope}/{code}/dates/$batchUpsert'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(requestBody, "{ [key: string]: CreateDateRequest; }")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: BatchUpsertDatesForCalendarResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "BatchUpsertDatesForCalendarResponse");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @summary DeleteDateFromCalendar: Remove a date from a calendar
     * @param scope Scope of the calendar
     * @param code Code of the calendar
     ...
     * @summary DeleteDatesFromCalendar: Delete dates from a calendar
     * @param scope Scope of the calendar
     * @param code Code of the calendar
     * @param requestBody Identifiers of the dates to be removed
     */
    public async deleteDatesFromCalendar (scope: string, code: string, requestBody: Array<string>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: { [key: string]: CalendarDate; };  }> {
        const localVarPath = this.basePath + '/api/calendars/generic/{scope}/{code}/dates/$delete'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: { [key: string]: CalendarDate; };  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "{ [key: string]: CalendarDate; }");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @summary GetCalendar: Get a calendar in its generic form
     ...
     * @summary ListCalendarsInScope: List all calenders in a specified scope
