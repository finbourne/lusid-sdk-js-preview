...
import { BlockAndOrderCreateRequest } from '../model/blockAndOrderCreateRequest';
...
import { ResourceListOfBlockAndOrder } from '../model/resourceListOfBlockAndOrder';
...
export class OrderManagementApi {
     ...
     * @param resourceId The allocations to create transactions for
     * @param applyFeesAndCommission Whether to apply fees and commissions to transactions (default: true)
     */
    public async bookTransactions (resourceId: Array<ResourceId>, applyFeesAndCommission?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: BookTransactionsResponse;  }> {
        ...
        if (resourceId === null || resourceId === undefined) {
            throw new Error('Required parameter resourceId was null or undefined when calling bookTransactions.');
        }
     ...
     * @summary [EARLY ACCESS] CreateOrders: Create Block and Order pairs
     * @param blockAndOrderCreateRequest The collection of block and order requests.
     */
    public async createOrders (blockAndOrderCreateRequest: BlockAndOrderCreateRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfBlockAndOrder;  }> {
        ...
        // verify required parameter 'blockAndOrderCreateRequest' is not null or undefined
        if (blockAndOrderCreateRequest === null || blockAndOrderCreateRequest === undefined) {
            throw new Error('Required parameter blockAndOrderCreateRequest was null or undefined when calling createOrders.');
        }
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(blockAndOrderCreateRequest, "BlockAndOrderCreateRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ResourceListOfBlockAndOrder;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ResourceListOfBlockAndOrder");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
