...
export class AggregationApi {
     ...
     * @summary [EXPERIMENTAL] GetQueryableKeys: Query the set of supported \"addresses\" that can be queried from the aggregation endpoint.
     ...
     * @summary [INTERNAL] SupportedAnalyticsInternal: Generates a matrix of analytics features supported by LUSID.
     * @param supportedAnalyticsInternalRequest 
     */
    public async supportedAnalyticsInternal (supportedAnalyticsInternalRequest?: SupportedAnalyticsInternalRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Aggregation;  }> {
        const localVarPath = this.basePath + '/api/aggregation/supportedanalyticsinternal';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        // give precedence to 'application/json'
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(supportedAnalyticsInternalRequest, "SupportedAnalyticsInternalRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Aggregation;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "Aggregation");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
