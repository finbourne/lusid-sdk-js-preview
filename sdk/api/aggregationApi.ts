...
import { QueryableKeysForMetricsRequest } from '../model/queryableKeysForMetricsRequest';
import { QueryableKeysForMetricsResponse } from '../model/queryableKeysForMetricsResponse';
...
export class AggregationApi {
     ...
     */
    public async getQueryableKeys (page?: string, limit?: number, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfAggregationQuery;  }> {
     ...
     * @summary [EXPERIMENTAL] GetQueryableKeysForMetrics: Query the queryable keys behind a given set of valuation metrics.
     * @param queryableKeysForMetricsRequest The set of metrics whose queryable keys are to be described
     */
    public async getQueryableKeysForMetrics (queryableKeysForMetricsRequest?: QueryableKeysForMetricsRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: QueryableKeysForMetricsResponse;  }> {
        const localVarPath = this.basePath + '/api/aggregation/$queryablekeys';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        // give precedence to 'application/json'
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(queryableKeysForMetricsRequest, "QueryableKeysForMetricsRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: QueryableKeysForMetricsResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "QueryableKeysForMetricsResponse");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
