...
export class PortfolioGroupsApi {
     ...
     */
    public async listPortfolioGroups (scope: string, effectiveAt?: string, asAt?: Date, filter?: string, relatedEntityPropertyKeys?: Array<string>, relationshipDefinitionIds?: Array<string>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfPortfolioGroup;  }> {
        const localVarPath = this.basePath + '/api/portfoliogroups/{scope}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
