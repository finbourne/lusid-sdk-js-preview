...
export class ReferencePortfolioApi {
     ...
     * @summary [EARLY ACCESS] UpsertReferencePortfolioConstituentProperties: Upsert constituent properties
     * @param scope The scope of the reference portfolio.
     * @param code The code of the reference portfolio. Together with the scope this uniquely identifies              the reference portfolio.
     * @param upsertReferencePortfolioConstituentPropertiesRequest The request to modify properties for the constituent.
     */
    public async upsertReferencePortfolioConstituentProperties (scope: string, code: string, upsertReferencePortfolioConstituentPropertiesRequest: UpsertReferencePortfolioConstituentPropertiesRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: UpsertReferencePortfolioConstituentPropertiesResponse;  }> {
        const localVarPath = this.basePath + '/api/referenceportfolios/{scope}/{code}/constituents/properties'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        // verify required parameter 'upsertReferencePortfolioConstituentPropertiesRequest' is not null or undefined
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(upsertReferencePortfolioConstituentPropertiesRequest, "UpsertReferencePortfolioConstituentPropertiesRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: UpsertReferencePortfolioConstituentPropertiesResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "UpsertReferencePortfolioConstituentPropertiesResponse");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
