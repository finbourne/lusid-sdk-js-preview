...
export class TranslationApi {
     ...
     * @summary [EXPERIMENTAL] TranslateInstrumentDefinitions: Translate instruments
     ...
     * @summary [EXPERIMENTAL] TranslateTradeTickets: Translate trade ticket
     * @param translateTradeTicketRequest The definitions of the trade ticket to translate along with the target dialect.
     */
    public async translateTradeTickets (translateTradeTicketRequest: TranslateTradeTicketRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TranslateTradeTicketsResponse;  }> {
        const localVarPath = this.basePath + '/api/translation/tradetickets';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        // give precedence to 'application/json'
        ...
        // verify required parameter 'translateTradeTicketRequest' is not null or undefined
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(translateTradeTicketRequest, "TranslateTradeTicketRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: TranslateTradeTicketsResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "TranslateTradeTicketsResponse");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
