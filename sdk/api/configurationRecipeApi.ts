...
export class ConfigurationRecipeApi {
     ...
     */
    public async listConfigurationRecipes (asAt?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfGetRecipeResponse;  }> {
     ...
     * @summary [EXPERIMENTAL] List the set of Configuration Recipes from all scopes
     * @param asAt The asAt datetime at which to list the Configuration Recipes. Defaults to latest if not specified.
     ...
     */
    public async listConfigurationRecipesForAllScopes (asAt?: Date, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfGetRecipeResponse;  }> {
        const localVarPath = this.basePath + '/api/recipes/list';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this.defaultHeaders);
        let localVarFormParams: any = {};
