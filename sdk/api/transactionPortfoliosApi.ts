 ...
 *
 * The version of the OpenAPI document: 2.0.1645
...
export class TransactionPortfoliosApi {
     ...
     * @summary [EARLY ACCESS] UpsertVirtualTransactionOverride: [EARLY ACCESS] Upsert a virtual transaction override
     * @param scope The scope of the transaction portfolio.
     * @param code The code of the transaction portfolio. Together with the scope this uniquely identifies              the transaction portfolio.
     * @param instrumentEventId The ID of the instrument event whose virtual transactions should be overridden.
     * @param transactionRequest A list of transactions to replace the virtual transactions generated by the instrument event.
     * @param portfolioEffectiveAt The effective datetime used to resolve the portfolio. Defaults to the current LUSID system datetime if not specified.
     * @param preserveProperties If set to false, the entire property set will be overwritten by the provided properties. If not specified or set to true, only the properties provided will be updated.
     * @param dataModelScope The optional scope of a Custom Data Model to use
     * @param dataModelCode The optional code of a Custom Data Model to use
     */
    public async upsertVirtualTransactionOverride (scope: string, code: string, instrumentEventId: string, transactionRequest: Array<TransactionRequest>, portfolioEffectiveAt?: string, preserveProperties?: boolean, dataModelScope?: string, dataModelCode?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: OverrideVirtualTransactionsResponse;  }> {
        const localVarPath = this.basePath + '/api/transactionportfolios/{scope}/{code}/overridevirtualtransactions'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        ...
        // verify required parameter 'transactionRequest' is not null or undefined
        if (transactionRequest === null || transactionRequest === undefined) {
            throw new Error('Required parameter transactionRequest was null or undefined when calling upsertVirtualTransactionOverride.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: OverrideVirtualTransactionsResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "OverrideVirtualTransactionsResponse");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
