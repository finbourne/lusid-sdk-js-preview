...
import { PortfolioCashFlowsResponse } from '../model/portfolioCashFlowsResponse';
...
let defaultBasePath = 'http://localhost:41479';
...
export class TransactionPortfoliosApi {
     ...
     */
    public async getPortfolioCashFlows (scope: string, code: string, fromEffectiveAt?: string, toEffectiveAt?: string, asAt?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PortfolioCashFlowsResponse;  }> {
        ...
        return new Promise<{ response: http.IncomingMessage; body: PortfolioCashFlowsResponse;  }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    reject(error);
                } else {
                    body = ObjectSerializer.deserialize(body, "PortfolioCashFlowsResponse");
                    if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                        resolve({ response: response, body: body });
                    } else {
                        reject({ response: response, body: body });
