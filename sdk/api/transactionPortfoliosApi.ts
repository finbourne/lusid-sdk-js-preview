...
export class TransactionPortfoliosApi {
     ...
     * @param instrumentIdentifierCollection The dictionary with the instrument identifiers to be updated on the              transaction and holdings.
     */
    public async resolveInstrument (scope: string, code: string, instrumentIdentifierType: string, instrumentIdentifierValue: string, fromEffectiveAt?: string, instrumentIdentifierCollection?: { [key: string]: string; }, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: UpsertPortfolioTransactionsResponse;  }> {
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(instrumentIdentifierCollection, "{ [key: string]: string; }")
        };
