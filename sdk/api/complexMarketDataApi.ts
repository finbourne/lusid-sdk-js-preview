...
export class ComplexMarketDataApi {
     ...
     * @summary DeleteComplexMarketData: Delete one or more items of complex market data, assuming they are present.
     ...
     * @param effectiveAt The effectiveAt datetime at which to list the ComplexMarketData. Defaults to latest if not specified. Note  that this parameter is not implemented at this time and the latest version of the ComplexMarketData will  always be returned.
     ...
     */
    public async listComplexMarketData (asAt?: Date, effectiveAt?: string, page?: string, limit?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfListComplexMarketDataWithMetaDataResponse;  }> {
        const localVarPath = this.basePath + '/api/complexmarketdata';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        // give precedence to 'application/json'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ResourceListOfListComplexMarketDataWithMetaDataResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ResourceListOfListComplexMarketDataWithMetaDataResponse");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @summary [EARLY ACCESS] UpsertAppendComplexMarketData: Appends a new point to the end of a ComplexMarketData definition.
     * @param scope The scope of the complex market data to append.
     * @param appendComplexMarketDataRequest Request definition of the point to append.
     * @param effectiveAt The effective datetime at which to retrieve the complex market data.               Defaults to the current LUSID system datetime if not specified.               Must match the effectiveAt of the ComplexMarketDataId given in the request body.
     * @param asAt The asAt datetime at which to retrieve the complex market data. Defaults to return the latest version if not specified.
     */
    public async upsertAppendComplexMarketData (scope: string, appendComplexMarketDataRequest: AppendComplexMarketDataRequest, effectiveAt?: string, asAt?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: UpsertSingleStructuredDataResponse;  }> {
        const localVarPath = this.basePath + '/api/complexmarketdata/{scope}/$append'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        ...
        // verify required parameter 'appendComplexMarketDataRequest' is not null or undefined
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(appendComplexMarketDataRequest, "AppendComplexMarketDataRequest")
        };
     ...
     * @summary UpsertComplexMarketData: Upsert a set of complex market data items. This creates or updates the data in Lusid.
