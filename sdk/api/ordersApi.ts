...
import { OrderSet } from '../model/orderSet';
...
export class OrdersApi {
     ...
     * @param asAt The asat datetime at which to retrieve the order. Defaults to              return the latest version of each order if not specified.
     ...
     */
    public async getOrder (scope: string, id: string, asAt?: Date, propertyKeys?: Array<string>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Order;  }> {
        const localVarPath = this.basePath + '/api/orders/{scope}/{id}'
     ...
     * @param asAt The asat datetime at which to retrieve the orders. Defaults to              return the latest version of each order if not specified.
     ...
     */
    public async listOrders (scope: string, asAt?: Date, propertyKeys?: Array<string>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfOrder;  }> {
        const localVarPath = this.basePath + '/api/orders/{scope}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this.defaultHeaders);
        let localVarFormParams: any = {};
        ...
        // verify required parameter 'scope' is not null or undefined
        if (scope === null || scope === undefined) {
            throw new Error('Required parameter scope was null or undefined when calling listOrders.');
        }
     ...
     */
    public async upsertOrders (scope: string, request?: OrderSetRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: OrderSet;  }> {
        const localVarPath = this.basePath + '/api/orders/{scope}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this.defaultHeaders);
        ...
        // verify required parameter 'scope' is not null or undefined
        if (scope === null || scope === undefined) {
            throw new Error('Required parameter scope was null or undefined when calling upsertOrders.');
        }
        ...
        return new Promise<{ response: http.IncomingMessage; body: OrderSet;  }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    reject(error);
                } else {
                    body = ObjectSerializer.deserialize(body, "OrderSet");
                    if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                        resolve({ response: response, body: body });
                    } else {
                        reject({ response: response, body: body });
