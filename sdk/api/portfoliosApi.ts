...
import { ResourceListOfReturn } from '../model/resourceListOfReturn';
...
export class PortfoliosApi {
     ...
     * @param period the type of the returns used to calculate the aggregation result.
     * @param outputFrequency The type of calculated output.
     ...
     * @param effectiveAt The effective datetime at which to list the property history. Defaults to the current LUSID system datetime if not specified.
     * @param asAt The asAt datetime at which to list the portfolio\&#39;s property history. Defaults to return the current datetime if not supplied.
     ...
     */
    public async getPortfolioPropertyTimeSeries (scope: string, code: string, propertyKey?: string, effectiveAt?: string, asAt?: Date, filter?: string, page?: string, limit?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfPropertyInterval;  }> {
     ...
     * @param asAt The asAt datetime at which to retrieve the Returns. Defaults to the latest.
     */
    public async getPortfolioReturns (scope: string, code: string, returnScope: string, returnCode: string, fromEffectiveAt?: string, toEffectiveAt?: string, period?: string, asAt?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfReturn;  }> {
        ...
        return new Promise<{ response: http.IncomingMessage; body: ResourceListOfReturn;  }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    reject(error);
                } else {
                    body = ObjectSerializer.deserialize(body, "ResourceListOfReturn");
                    if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                        resolve({ response: response, body: body });
                    } else {
                        reject({ response: response, body: body });
     ...
     * @param requestBody The properties to be updated or inserted onto the portfolio. Each property in               the request must be keyed by its unique property key. This has the format {domain}/{scope}/{code} e.g. \&quot;Portfolio/Manager/Id\&quot;.
     */
     ...
     * @param returnScope The scope of the Returns.
     * @param returnCode The code of the Returns.
     * @param _return This contains the Returns which need to be upsert.
     */
    public async upsertPortfolioReturns (scope: string, code: string, returnScope: string, returnCode: string, _return: Array<Return>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: UpsertReturnsResponse;  }> {
        const localVarPath = this.basePath + '/api/portfolios/{scope}/{code}/returns/{returnScope}/{returnCode}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)))
            .replace('{' + 'returnScope' + '}', encodeURIComponent(String(returnScope)))
        ...
        if (_return === null || _return === undefined) {
            throw new Error('Required parameter _return was null or undefined when calling upsertPortfolioReturns.');
        }
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(_return, "Array<Return>")
        };
