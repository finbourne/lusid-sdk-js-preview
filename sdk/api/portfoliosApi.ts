...
export class PortfoliosApi {
     ...
     * @summary [EXPERIMENTAL] PatchPortfolio: Patch portfolio.
     * @param scope The scope of the portfolio.
     * @param code The code of the portfolio. Together with the               scope this uniquely identifies the portfolio.
     * @param operation The json patch document. For more check: https://datatracker.ietf.org/doc/html/rfc6902.
     */
    public async patchPortfolio (scope: string, code: string, operation: Array<Operation>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ActionResultOfPortfolio;  }> {
        const localVarPath = this.basePath + '/api/portfolios/{scope}/{code}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ActionResultOfPortfolio;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ActionResultOfPortfolio");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @summary [EXPERIMENTAL] PatchPortfolioAccessMetadata: Patch Access Metadata rules for a Portfolio.
     ...
     */
    public async patchPortfolioAccessMetadata (scope: string, code: string, operation: Array<Operation>, effectiveAt?: string, effectiveUntil?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: { [key: string]: Array<AccessMetadataValue>; };  }> {
        const localVarPath = this.basePath + '/api/portfolios/{scope}/{code}/metadata'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        ...
        // verify required parameter 'operation' is not null or undefined
        if (operation === null || operation === undefined) {
            throw new Error('Required parameter operation was null or undefined when calling patchPortfolioAccessMetadata.');
        }
