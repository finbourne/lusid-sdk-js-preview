...
export class FundsApi {
     ...
     */
    public async createFee (scope: string, code: string, feeCode: string, feeRequest: FeeRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Fee;  }> {
        const localVarPath = this.basePath + '/api/funds/{scope}/{code}/fee/{feeCode}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)))
        ...
        if (feeCode === null || feeCode === undefined) {
            throw new Error('Required parameter feeCode was null or undefined when calling createFee.');
        }
    ...
    public async listFees (scope: string, code: string, effectiveAt?: string, asAt?: Date, page?: string, limit?: number, filter?: string, sortBy?: Array<string>, propertyKeys?: Array<string>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PagedResourceListOfFee;  }> {
        const localVarPath = this.basePath + '/api/funds/{scope}/{code}/fee'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
