...
export class FundsApi {
     ...
     * @summary [EXPERIMENTAL] GetHoldingContributorsForFund: Get holdings contributors for transaction portfolios in a Fund.
     * @param scope The scope of the Fund.
     * @param code The code of the Fund. Together with the scope this uniquely identifies the Fund.
     * @param holdingId The unique holding identifier
     * @param singleValuationPointQueryParameters The arguments to use for querying the holdings.This can be a date, valuationPoint or a bookmark.
     ...
     */
    public async getHoldingContributorsForFund (scope: string, code: string, holdingId: number, singleValuationPointQueryParameters: SingleValuationPointQueryParameters, navTypeCode?: string, fromTradeDate?: string, toTradeDate?: string, includeHistoric?: boolean, taxLotId?: string, includeUnsettledMovements?: boolean, limit?: number, asAt?: Date, page?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: VersionedResourceListOfHoldingContributor;  }> {
        ...
        if (singleValuationPointQueryParameters === null || singleValuationPointQueryParameters === undefined) {
            throw new Error('Required parameter singleValuationPointQueryParameters was null or undefined when calling getHoldingContributorsForFund.');
        }
     ...
     * @param valuationPointCodeVariant The variant of the valuation point used in the request. Together with the valuation point code marks the unique branch for the NavType.
     */
    public async upsertNavActivityAdjustments (scope: string, code: string, valuationPointCode: string, navTypeCode: string, navActivityAdjustment: Array<NavActivityAdjustment>, valuationPointCodeVariant?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Date;  }> {
        const localVarPath = this.basePath + '/api/funds/{scope}/{code}/navAdjustment'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
