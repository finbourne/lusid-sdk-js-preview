...
export class AborApi {
     ...
     * @summary [EXPERIMENTAL] GetJELines: Get the JELines for the given Abor.
     * @param scope The scope of the Abor.
     ...
     */
    public async getJELines (scope: string, code: string, jELinesQueryParameters: JELinesQueryParameters, asAt?: Date, limit?: number, page?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: VersionedResourceListOfJELines;  }> {
        const localVarPath = this.basePath + '/api/abor/{scope}/{code}/JELines/$query'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: VersionedResourceListOfJELines;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "VersionedResourceListOfJELines");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
