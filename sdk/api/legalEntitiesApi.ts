...
export class LegalEntitiesApi {
     ...
     */
    public async patchLegalEntityAccessMetadata (idTypeScope: string, idTypeCode: string, code: string, operation: Array<Operation>, effectiveAt?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: { [key: string]: Array<AccessMetadataValue>; };  }> {
        const localVarPath = this.basePath + '/api/legalentities/{idTypeScope}/{idTypeCode}/{code}/metadata'
            .replace('{' + 'idTypeScope' + '}', encodeURIComponent(String(idTypeScope)))
            .replace('{' + 'idTypeCode' + '}', encodeURIComponent(String(idTypeCode)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
