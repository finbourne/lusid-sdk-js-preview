...
export class InstrumentEventTypesApi {
     ...
     * @param instrumentType The template is applied to events which originate from instruments of this type
     ...
     */
    public async getTransactionTemplate (instrumentEventType: string, scope?: string, asAt?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TransactionTemplate;  }> {
        const localVarPath = this.basePath + '/api/instrumenteventtypes/{instrumentEventType}/transactiontemplates'
            .replace('{' + 'instrumentEventType' + '}', encodeURIComponent(String(instrumentEventType)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
