...
export enum AddressKeyDefinitionApiApiKeys {
}

export class AddressKeyDefinitionApi {
    ...
    public setApiKey(key: AddressKeyDefinitionApiApiKeys, value: string) {
        (this.authentications as any)[AddressKeyDefinitionApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async createAddressKeyDefinition (createAddressKeyDefinitionRequest: CreateAddressKeyDefinitionRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: AddressKeyDefinition;  }> {
        const localVarPath = this.basePath + '/api/addresskeydefinitions';
        ...
        // verify required parameter 'createAddressKeyDefinitionRequest' is not null or undefined
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(createAddressKeyDefinitionRequest, "CreateAddressKeyDefinitionRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: AddressKeyDefinition;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "AddressKeyDefinition");
     ...
     */
    public async getAddressKeyDefinition (key: string, asAt?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: AddressKeyDefinition;  }> {
        const localVarPath = this.basePath + '/api/addresskeydefinitions/{key}'
     ...
     */
    public async listAddressKeyDefinitions (asAt?: Date, page?: string, limit?: number, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PagedResourceListOfAddressKeyDefinition;  }> {
        const localVarPath = this.basePath + '/api/addresskeydefinitions';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PagedResourceListOfAddressKeyDefinition;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PagedResourceListOfAddressKeyDefinition");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
