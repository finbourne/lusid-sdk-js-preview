...
export enum QueryableKeysApiApiKeys {
}

export class QueryableKeysApi {
    ...
    public setApiKey(key: QueryableKeysApiApiKeys, value: string) {
        (this.authentications as any)[QueryableKeysApiApiKeys[key]].apiKey = value;
    }
     ...
     * @summary [EARLY ACCESS] GetAllQueryableKeys: Query the set of supported \"addresses\" that can be queried from all endpoints.
     * @param asAt For user defined DerivedValuation keys.
     * @param filter Expression to filter the result set.              Read more about filtering results from LUSID here https://support.lusid.com/filtering-results-from-lusid.
     */
    public async getAllQueryableKeys (asAt?: Date, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfQueryableKey;  }> {
        const localVarPath = this.basePath + '/api/queryablekeys';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ResourceListOfQueryableKey;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ResourceListOfQueryableKey");
