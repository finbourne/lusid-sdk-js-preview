...
export class QuotesApi {
     ...
     */
    public async upsertQuoteAccessMetadataRule (scope: string, upsertQuoteAccessMetadataRuleRequest: UpsertQuoteAccessMetadataRuleRequest, effectiveAt?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: QuoteAccessMetadataRule;  }> {
        const localVarPath = this.basePath + '/api/metadata/quotes/rules/{scope}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
