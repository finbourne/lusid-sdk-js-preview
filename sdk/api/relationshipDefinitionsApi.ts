...
export class RelationshipDefinitionsApi {
     ...
     * @summary [EXPERIMENTAL] CreateRelationshipDefinition: Create Relationship Definition
     ...
     * @summary [EXPERIMENTAL] DeleteRelationshipDefinition: Delete Relationship Definition
     * @param scope The scope of the relationship to be deleted.
     * @param code The code of the relationship to be deleted. Together with the domain and scope this uniquely              identifies the relationship.
     */
    public async deleteRelationshipDefinition (scope: string, code: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DeletedEntityResponse;  }> {
     ...
     * @summary [EXPERIMENTAL] GetRelationshipDefinition: Get relationship definition
     ...
     * @summary [EXPERIMENTAL] ListRelationshipDefinitions: List relationship definitions
     ...
     */
    public async listRelationshipDefinitions (asAt?: Date, page?: string, limit?: number, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PagedResourceListOfRelationshipDefinition;  }> {
        const localVarPath = this.basePath + '/api/relationshipdefinitions';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PagedResourceListOfRelationshipDefinition;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PagedResourceListOfRelationshipDefinition");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @summary [EXPERIMENTAL] UpdateRelationshipDefinition: Update Relationship Definition
