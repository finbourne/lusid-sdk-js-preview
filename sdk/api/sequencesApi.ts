...
export class SequencesApi {
     ...
     * @summary [EXPERIMENTAL] CreateSequence: Create a new sequence
     ...
     * @summary [EXPERIMENTAL] GetSequence: Get a specified sequence
     * @param scope Scope of the sequence.
     ...
     * @summary [EXPERIMENTAL] ListSequences: List Sequences
     ...
     */
    public async listSequences (page?: string, limit?: number, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PagedResourceListOfSequenceDefinition;  }> {
        const localVarPath = this.basePath + '/api/sequences';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        // give precedence to 'application/json'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PagedResourceListOfSequenceDefinition;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PagedResourceListOfSequenceDefinition");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @summary [EXPERIMENTAL] Next: Get next values from sequence
     * @param scope Scope of the sequence.
     * @param code Code of the sequence. This together with stated scope uniquely              identifies the sequence.
