...
export class CorporateActionSourcesApi {
     ...
     */
    public async getInstrumentEvents (scope: string, code: string, asAt?: Date, limit?: number, page?: string, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfInstrumentEventHolder;  }> {
        const localVarPath = this.basePath + '/api/corporateactionsources/{scope}/{code}/instrumentevents'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
