...
import { DeletedOrdersEntitiesResponse } from '../model/deletedOrdersEntitiesResponse';
...
export class AllocationsApi {
     ...
     * @summary [EXPERIMENTAL] Delete Allocations.
     * @param resourceId The IDs of the allocations to delete.
     */
    public async deleteAllocations (resourceId: Array<ResourceId>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DeletedOrdersEntitiesResponse;  }> {
        const localVarPath = this.basePath + '/api/allocations/$delete';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this.defaultHeaders);
        let localVarFormParams: any = {};
        ...
        if (resourceId === null || resourceId === undefined) {
            throw new Error('Required parameter resourceId was null or undefined when calling deleteAllocations.');
        }
        ...
        return new Promise<{ response: http.IncomingMessage; body: DeletedOrdersEntitiesResponse;  }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    reject(error);
                } else {
                    body = ObjectSerializer.deserialize(body, "DeletedOrdersEntitiesResponse");
                    if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                        resolve({ response: response, body: body });
                    } else {
                        reject({ response: response, body: body });
