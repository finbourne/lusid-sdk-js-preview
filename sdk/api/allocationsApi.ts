...
export class AllocationsApi {
     ...
     * @summary ListAllocations: List Allocations
     * @param asAt The asAt datetime at which to retrieve the allocation. Defaults to return the latest version of the allocation if not specified.
     ...
     * @param propertyKeys A list of property keys from the \&quot;Allocations\&quot; domain to decorate onto each allocation.                  These take the format {domain}/{scope}/{code} e.g. \&quot;Allocations/system/Name\&quot;.                  All properties, except derived properties, are returned by default, without specifying here.
     */
    public async listAllocations (asAt?: Date, page?: string, sortBy?: Array<string>, limit?: number, filter?: string, propertyKeys?: Array<string>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PagedResourceListOfAllocation;  }> {
     ...
     * @summary UpsertAllocations: Upsert Allocations
     * @param allocationSetRequest The collection of allocation requests.
     * @param verificationAsAt An optional verification asAt; individual upserts will fail if an existing entity has been updated between the verification asAt and time of upsert.
     ...
     */
    public async upsertAllocations (allocationSetRequest: AllocationSetRequest, verificationAsAt?: Date, retryWithoutChangedEntities?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfAllocation;  }> {
        const localVarPath = this.basePath + '/api/allocations';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        ...
        // verify required parameter 'allocationSetRequest' is not null or undefined
        ...
        if (verificationAsAt !== undefined) {
            localVarQueryParameters['verificationAsAt'] = ObjectSerializer.serialize(verificationAsAt, "Date");
        }

        if (retryWithoutChangedEntities !== undefined) {
            localVarQueryParameters['retryWithoutChangedEntities'] = ObjectSerializer.serialize(retryWithoutChangedEntities, "boolean");
        }
