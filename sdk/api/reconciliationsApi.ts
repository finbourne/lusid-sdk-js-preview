...
import { AggregationReconciliation } from '../model/aggregationReconciliation';
...
export class ReconciliationsApi {
     ...
     */
    public async reconcileInline (sortBy?: Array<string>, start?: number, limit?: number, filter?: string, inlineValuationsReconciliationRequest?: InlineValuationsReconciliationRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: AggregationReconciliation;  }> {
        ...
        return new Promise<{ response: http.IncomingMessage; body: AggregationReconciliation;  }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    reject(error);
                } else {
                    body = ObjectSerializer.deserialize(body, "AggregationReconciliation");
                    if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                        resolve({ response: response, body: body });
                    } else {
                        reject({ response: response, body: body });
     ...
     */
    public async reconcileValuation (sortBy?: Array<string>, start?: number, limit?: number, filter?: string, valuationsReconciliationRequest?: ValuationsReconciliationRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfReconciliationBreak;  }> {
