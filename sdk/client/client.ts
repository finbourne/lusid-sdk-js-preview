...
// What Authenication method to use.  Defaults to the historical value of RefreshingToken.
// PersonalAccessToken's are used for testing and in tightly controlled circumstances.  These tokens
// are not refreshed.
...
class Credentials {
  ...
  constructor(tokenUrl: string, username: string, password: string, clientId: string, clientSecret: string) {
...
export class LusidClient {
  ...
  // Personal Access Token (PAT) override.
  private personalAccessTokenDetails: [Source, string];
  ...
  constructor(
    ...
    personalAccessTokenDetails?: [Source, string]
  ) {
    // Set the path to the secrets file
    this.secretsFilePath = secretsPath;

    // Set the authentication method.  In the absense of an expilicitly set method, use the method used historically
    this.authMethod = authenticationMethod ?? AuthenticationMethod.RefreshingToken;
    ...
    // if the authenication method states PAT then set the token.
    if (this.authMethod == AuthenticationMethod.PersonalAccessToken) {
      this.personalAccessTokenDetails = personalAccessTokenDetails;
    }
    ...
    lusid.APIS.forEach((api: any) => {
      ...
      for (var prop in this.api[apiName]) {
        ...
        if (typeof this.api[apiName][prop] == "function" && !["setDefaultAuthentication", "setApiKey"].includes(prop)) {
          ...
          this.api[apiName][prop] = this.apiFunctionWrapper(this.api[apiName][prop], this.api[apiName], this);
        }
      }
    });
  }
      ...
      switch (self.authMethod) {
        case AuthenticationMethod.PersonalAccessToken:
          return new Promise(function (resolve, reject) {
            ...
            api["authentications"]["oauth2"]["accessToken"] = self.fetchCredentials(
              self.personalAccessTokenDetails[0],
              self.personalAccessTokenDetails[1]
            );
            /*
        ...
        case AuthenticationMethod.RefreshingToken:
          ...
          return new Promise(function (resolve, reject) {
            ...
            oauthPopulated
              .then(function (oauth2Details: Oauth2) {
        ...
        default:
          throw "AuthenticationMethod should be set to the default (RefreshingToken) at the very least by now.";
      }
    };
  }
  ...
  */
  private checkTokenRefresh(oauth2: Oauth2, refreshLimit: number): boolean {
